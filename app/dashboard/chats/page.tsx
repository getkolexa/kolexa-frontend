import { Button } from "@/components/ui/button"
import { Search, Filter, MessageSquare, Plus, Clock, ChevronDown, FileText } from "lucide-react"
import ChatCard from "@/components/dashboard/chat-card"

export const metadata = {
  title: "Chats - KOLEXAI",
  description: "Your AI learning conversations",
}

export default function ChatsPage() {
  const chats = [
    {
      id: 1,
      title: "Blockchain Fundamentals",
      lastMessage: "Can you explain how consensus mechanisms work in blockchain?",
      lastActive: "2 hours ago",
      messages: 24,
      library: "Blockchain",
    },
    {
      id: 2,
      title: "Smart Contract Security",
      lastMessage: "What are the common vulnerabilities in smart contracts?",
      lastActive: "Yesterday",
      messages: 18,
      library: "Smart Contracts",
    },
    {
      id: 3,
      title: "Web3 Development",
      lastMessage: "How do I connect my frontend to a blockchain using ethers.js?",
      lastActive: "3 days ago",
      messages: 32,
      library: "Web3 Development",
    },
    {
      id: 4,
      title: "DeFi Principles",
      lastMessage: "What is liquidity mining and how does it work?",
      lastActive: "1 week ago",
      messages: 15,
      library: "DeFi Principles",
    },
  ]

  return (
    <div className="max-w-6xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-3">
        <div>
          <h1 className="text-sm font-semibold text-white mb-1">AI Chats</h1>
          <p className="text-xs text-gray-400">Your conversations with the AI learning assistant</p>
        </div>

        <div className="flex items-center space-x-2">
          <div className="relative">
            <div className="absolute inset-y-0 left-0 pl-2 flex items-center pointer-events-none">
              <Search className="h-3 w-3 text-gray-400" />
            </div>
            <input
              type="text"
              placeholder="Search chats..."
              className="py-1 pl-7 pr-3 block w-full text-xs rounded-md focus:outline-none focus:ring-1 focus:ring-purple-500 bg-slate-800/50 border border-white/10 text-white placeholder-gray-400"
            />
          </div>

          <Button variant="outline" size="sm" className="h-6 text-[10px] border-white/10 text-white hover:bg-white/5">
            <Filter className="h-3 w-3 mr-1" />
            Filter
          </Button>

          <Button
            size="sm"
            className="h-6 text-[10px] bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 text-white border-0"
          >
            <Plus className="h-3 w-3 mr-1" />
            New Chat
          </Button>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {chats.map((chat) => (
          <ChatCard key={chat.id} chat={chat} />
        ))}

        <div className="bg-slate-900/50 border border-dashed border-white/10 rounded-lg p-4 flex flex-col items-center justify-center h-48">
          <div className="h-10 w-10 rounded-full bg-slate-800/50 flex items-center justify-center mb-2">
            <MessageSquare className="h-5 w-5 text-gray-400" />
          </div>
          <p className="text-xs text-white mb-1">Start New Chat</p>
          <p className="text-[10px] text-gray-400 text-center mb-3">Ask questions about any topic</p>
          <Button variant="outline" size="sm" className="h-6 text-[10px] border-white/10 text-white hover:bg-white/5">
            <Plus className="h-3 w-3 mr-1" />
            New Chat
          </Button>
        </div>
      </div>

      <div className="mt-6">
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-xs font-medium text-white">Recent Activity</h2>
          <div className="flex items-center">
            <Button variant="ghost" size="sm" className="h-6 text-[10px] text-gray-400 hover:text-white">
              <Clock className="h-3 w-3 mr-1" />
              Last 7 days
              <ChevronDown className="h-3 w-3 ml-1" />
            </Button>
          </div>
        </div>

        <div className="bg-slate-900/80 border border-white/10 rounded-lg overflow-hidden">
          <div className="grid grid-cols-[auto_1fr_auto_auto_auto] gap-3 p-2 border-b border-white/5 text-[10px] font-medium text-gray-400">
            <div className="pl-2">Type</div>
            <div>Chat</div>
            <div>Library</div>
            <div>Date</div>
            <div className="pr-2">Actions</div>
          </div>

          <div className="divide-y divide-white/5">
            {[
              { type: "chat", name: "Blockchain Fundamentals", library: "Blockchain", date: "2 hours ago" },
              { type: "chat", name: "Smart Contract Security", library: "Smart Contracts", date: "Yesterday" },
              { type: "document", name: "Web3 Development Notes", library: "Web3 Development", date: "2 days ago" },
              { type: "chat", name: "DeFi Principles", library: "DeFi Principles", date: "3 days ago" },
              { type: "document", name: "Blockchain Security Guide", library: "Blockchain", date: "4 days ago" },
            ].map((item, i) => (
              <div key={i} className="grid grid-cols-[auto_1fr_auto_auto_auto] gap-3 p-2 hover:bg-slate-800/30 text-xs">
                <div className="pl-2">
                  {item.type === "chat" ? (
                    <MessageSquare className="h-4 w-4 text-blue-400" />
                  ) : (
                    <FileText className="h-4 w-4 text-green-400" />
                  )}
                </div>
                <div className="text-white truncate">{item.name}</div>
                <div className="text-gray-400">{item.library}</div>
                <div className="text-gray-400">{item.date}</div>
                <div className="pr-2 flex items-center space-x-1">
                  <Button variant="ghost" size="icon" className="h-5 w-5 text-gray-400 hover:text-white">
                    <MessageSquare className="h-3 w-3" />
                  </Button>
                </div>
              </div>
            ))}
          </div>
        </div>
      </div>
    </div>
  )
}
